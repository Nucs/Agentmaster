<!-- Agentmaster 0.6.5 release notes — PRE-RELEASE (NOT Latest; v0.6.1 stays Latest).
     REUNIFICATION prerelease: mainline tip d28a9b252, 30 commits past v0.6.1 (f0df61f96).
     Folds in the full (previously "baking") 0.6.2 batch + the 0.6.3 leak/updater hotfix +
     the 0.6.4 resume-crash-loop fix + 7 newer state/triage/noise-gate commits.
     Last stable is 0.6.1, so this IS the full changelog since last stable.
...
**0.6.5 is a pre-release that reunifies the 0.6.x line.** After 0.6.1, the large tab / overlay / restart batch was held back to bake — it shipped only as the abandoned **0.6.2** pre-release — while two isolated hotfixes went out on their own: the **23 GB Manager memory-leak fix** and an **hourly update check** (0.6.3), then the **resume crash-loop fix** (0.6.4). 0.6.5 brings **all of it together** on top of 0.6.1 — the full 0.6.2 batch, both hotfix lines, **and** newer session-state, triage, and noise-gate work — as one pre-release. Enable **Settings → Updates → "Allow pre-release versions"** to receive it automatically, or install from the assets below. The stable **0.6.1** remains the default for everyone else.
