...
Atomic prompt delivery (the DELIVERY GATE — one owner per input box; advance/watchdog/swap serialization): [`doc/agentmaster/DELIVERY.md`](doc/agentmaster/DELIVERY.md) (+ the residuals R1–R3 — lost-send reconciler / sound pickup evidence + the Enter-retry DRAFT GUARD / empty-phantom-UPS hardening — IMPLEMENTED, outcomes in §10; + the **R4–R8 VERIFIED PLACEMENT** wave — **now IMPLEMENTED too** (outcome in §11; plan + deviations: [`doc/agentmaster/DELIVERY_PLAN.md`](doc/agentmaster/DELIVERY_PLAN.md) Part 2; engine-tested `TestVerifiedPlacement`, harness 3214/3214, libs + CLI green, rides the next deploy cycle), motivated by **Incident 3** (§11: a 13-char prompt merged with ~4.2K of invisible TUI-internal content through the no-draft fast path — every read said empty, the submitted message read 4205 chars). It closed the one UN-validated write: the send was paste+CR in ONE blind write and is now **FILL → READ-BACK-VERIFY → CR** — `VerifyFillAgainstPrompt` (whitespace-stripped so soft wrap can't false-Foreign; collapse-aware so the handover paste tier's `[Pasted text #N +M lines]` render verifies) gates the lone CR; `Eaten` re-fills ≤2, `Partial` discards-own-text + re-fills, `Foreign` = **the merge caught pre-commit** (verified backspace-batch undo, budget == folded prompt length, remainder recorded as pendingInput), 2 unverifiable deliveries ⇒ the prompt FAILS terminally (the verify-STRIKE ledger — a plain rollback would livelock re-verifying; since §12 the strike keeps the autorunner MODE — a refused merge is the protection working, and the fills rose to ≤3) — off-switch `verifySendBeforeSubmit`, default ON (R4). The box read is tri-state now — `InputBoxState` Unknown/NoBox/Empty/Draft/MenuOpen through the ControlCore boundary (`ReadPendingInputBoxState` on the cached scan + the deep `ReadInputBoxProbe(rows)`, since a filled prompt can outgrow the 120-row window): the swap PRE-FLIGHT declines into NoBox/MenuOpen (a paste would feed the MENU — the §9 damage shape), `DecideAdvance` holds on the recorded state (the release NOTIFIES, no polling), a 60s-persistent NoBox with queued work WARNS loudly once per episode (`ShouldWarnOnBoxNotVisible` — §12 defanged the R5 mode→Off pause after it overrode a manual Off→Full re-arm off a clock predating the human's GO; the hold already self-resumes when a box renders, and a manual re-arm now RE-STAMPS the NoBox clock on both arming surfaces), and the fill pumps treat only Empty/Draft as "known" (R5). A failed stash-restore can no longer park a landmine in claude's auto-popping slot (progress-extending settle + the junk-kill-path un-stash rung leaves popped content VISIBLE; the R6c rung-default flip stays probe-gated). A merged submit is NAMED `[merge-detected]` at BOTH echo seams (push UPS + pull NoteExternalPrompt) and Failed immediately instead of surfacing as an 18s-late lost-send (R7). The watchdog's lone Enter routes through a live-read VERIFIED PRESSER — presses only into verified-Empty or the watched prompt, never a menu (Enter would SELECT an option) — and the wiring closed a pre-existing gap: the adopt-bind path registered the injector ALONE (no submitter ⇒ adopted sessions had NO draft swap and NO verify); launched + adopted sessions now share one protection contract (R8); + the **§12 CONFIRM-AND-RETRY wave — Incident 4** (IMPLEMENTED, harness 3244/3244, lib green, rides the next deploy cycle): two of the new give-up-to-Off paths fired live on FALSE signals — the R5 escalation overrode a manual Full 48s after the user set it (its clock predated the re-arm), and the lost-send verdict Failed a prompt ONE second after gate expiry whose delivery was still queued behind a restore-wedged dispatcher (mark-Sent→injection measured 83s apart; the "lost" prompt then delivered `verified=exact` and double-recorded as Typed). The lost verdict now requires **INJECTION EVIDENCE** (`QueuedPrompt::injectedAtUnixMs`, stamped at every `[delivered]` seam via `MarkPromptInjected`, settle from max(sentAt, injectedAt)); a Sent-but-never-injected prompt whose gate claim lapsed is **RECLAIMED to Pending** and retried (`DecideUndeliveredReclaim` → `[send-reclaim]` — nothing was typed, nothing can double, the mode is untouched); each delivery ATTEMPT owns a unique gate tag (`PromptSubmission::submitNonce` → `promptId#nonce`) and the swap re-asserts its claim at the top guard, each fill, and the CR commit (`RevalidateDeliveryGate` — an expired-but-unreclaimed claim revives, a reclaimed one aborts the stale carrier with nothing typed), so a reclaim + resend and its zombie predecessor can never both deliver); + the **§13 wave — Incident 5, the ts-corrupted duplicate-Stop DOUBLE-SEND** (IMPLEMENTED, harness 3313/3313, lib green, rides the next deploy cycle): session `ea1dc3dd` (2026-08-15) had two queued prompts delivered 2.6s apart — a DUPLICATE delivery of an already-processed Stop arrived 37ms after prompt #1's UPS echo with its trailing wire `ts` lost to truncation, and **`ts==0` DISABLES both of `NextSessionStateOrdered`'s ordering defenses** (the stale check AND the `kMinRealTurnSpanMs` floor are gated on `ts != 0`), so it read as a fresh turn-complete and the advance sent #2 into the RUNNING turn (claude queued it, the user's box-clear orphaned it, enter-retry gave up). TWO fixes: **the bridge stamps ARRIVAL time on a ts-less parse** (`HooksBridge.cpp`, both dispatch sites — arrival can only OVERSTATE the fire time, erring toward stale/floored, the self-healing direction) and the **MINIMUM SEND SPACING** (`Scheduler.h kMinSendSpacingMs` **20s** — never two AUTOMATIC injections into one session closer than 20s, measured from OUR OWN registry-clock stamps (`sentAtUnixMs` on Sent/Failed Autorun rows + `injectedAtUnixMs` on any) so NO wire corruption can defeat it; every status-correction lane needs only seconds, so 20s converts a status race from a DOUBLE send into a merely DELAYED one. Typed rows + rolled-back Pendings + the human's explicit paths (Send-now, SemiAuto Confirm) are deliberately NOT spaced; the hold is a TIMED None — `AdvancePlan::retryAfterMs` → the scheduler's deferred re-advance (`_scheduleDeferredAdvance`, the worker wakes at the earliest due), so a spaced plan self-resumes with no external notify. ⚠ deliberate behavior change: an echoed lead no longer releases the next send IMMEDIATELY — the echo still releases the pickup guard, but the next injection waits out the spacing window).
...
- **Engine (M5, `AgentMaster/`; M9 process singleton).** Thread-safe `SessionRegistry` (single
  ...
  **pickup guard is EVIDENCE-released** now, not
  ...
  is at rest + settled 15s with NO echo either way — **and with INJECTION EVIDENCE**
  (`injectedAtUnixMs`, stamped at every `[delivered]` seam; settle from max(sentAt, injectedAt) —
  DELIVERY.md §12: without the conjunct the verdict fired on a delivery still queued behind a
  wedged dispatcher, one second after gate expiry, and the "lost" prompt then delivered
  `verified=exact`) — is judged **LOST** by the scanner's pass (pure `DecideLostSend`,
  `kLostSendSettleMs`) → **`Failed` + that session's autorunner paused**, logged `[lost-send]`,
  never auto-resent — the `#6` "delivered but never became a message" gap closed; a
  Sent-but-never-INJECTED prompt whose gate claim lapsed is instead **RECLAIMED to Pending** and
  retried (`DecideUndeliveredReclaim` → `[send-reclaim]`, mode untouched — nothing was typed, so
  nothing can double; the stale delivery aborts on its per-attempt nonce gate tag);
...
- **Tests Autorunner (M7, `Scheduler`).** Pure `DecideAdvance()` + a worker thread on the registry
  ...
  **started**, re-presses a **lone Enter** (never the text again — it is already typed; resending
  ...
  15s settle at rest, injection-evidence-gated since §12 → `Failed` + autorunner paused +
  `[lost-send]`, never resent — R1).
