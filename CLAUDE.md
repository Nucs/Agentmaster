...
Atomic prompt delivery (the DELIVERY GATE — one owner per input box; advance/watchdog/swap serialization): [`doc/agentmaster/DELIVERY.md`](doc/agentmaster/DELIVERY.md) (+ the residuals R1–R3 — lost-send reconciler / sound pickup evidence + the Enter-retry DRAFT GUARD / empty-phantom-UPS hardening — now IMPLEMENTED, outcomes in §10, plan + deviations: [`doc/agentmaster/DELIVERY_PLAN.md`](doc/agentmaster/DELIVERY_PLAN.md); the NEXT wave — **R4–R8 VERIFIED PLACEMENT**, PLANNED in DELIVERY_PLAN.md Part 2 off **Incident 3** (§11: a 13-char prompt merged with ~4.2K of invisible TUI-internal content through the no-draft fast path — every read said empty, the submitted message read 4205 chars) — closes the one UN-validated write: the send commits paste+CR in ONE blind write, so it becomes **FILL → READ-BACK-VERIFY → CR** with recovery per arrow (R4), the box read becomes tri-state — `boxFound` exists in the pure detector but is DROPPED at the ControlCore boundary, so "menu open / box invisible" reads as "empty ⇒ safe" (R5), a failed stash/kill round-trip must never leave content parked in claude's auto-popping channels (R6 — the incident's fuel), a merged submit gets NAMED at the echo seam instead of surfacing as a 18s-late lost-send (R7), and the watchdog's lone Enter goes live-read + menu-refusing — today it can SELECT an AskUserQuestion option when recon-block lags (R8)).
