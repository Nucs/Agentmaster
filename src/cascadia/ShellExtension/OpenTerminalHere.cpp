...
try
{
...

    {
        ...
        std::wstring cmdline;
        cmdline.reserve(256); // The Agentmaster.exe path is ~110 characters long
        cmdline.push_back(L'"');
...
{
    std::filesystem::path modulePath{ wil::GetModuleFileNameW<std::wstring>(wil::GetModuleInstanceHandle()) };
    // Agentmaster.exe,-101 will be the first icon group in the app exe (the renamed
    // WindowsTerminal project output). We're using the GUI binary here explicitly, and not the
    // alias (from GetWtExePath), because it is the only one built with the right icons.
    modulePath.replace_filename(L"Agentmaster.exe,-101");
