...
namespace winrt::TerminalApp::implementation
{
    ...
    public:
        ...
        void SetNewSessionModels(const std::vector<std::pair<std::wstring, std::wstring>>& models, bool isCodex); // Agentmaster (launch-model picker): swap the plain "New Session Here" item for a submenu — Default + one item per configured {displayName, modelId} — and (re)populate it; page-driven at flyout-open from ParseLaunchModels(AppSettings.launchModels). isCodex (or an empty list) keeps the PLAIN item (the models are Claude models — a codex spawn takes no --model)
        void SetForkSessionModels(const std::vector<std::pair<std::wstring, std::wstring>>& models, bool isManagedClaude); // Agentmaster (launch-model picker): same swap for "Fork session" — the submenu (Default + models) shows ONLY for a managed CLAUDE tab with a non-empty list (a fork is a launch, so --model applies); a Codex tab (codex fork takes no --model) and a plain shell tab (whose "Fork session" is really WT's duplicate-tab) keep the PLAIN item. Page-driven at flyout-open
