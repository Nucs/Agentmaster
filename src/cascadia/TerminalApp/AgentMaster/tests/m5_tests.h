...
void TestVerifiedPlacement(); // DELIVERY_PLAN.md Part 2 (R4-R8): the tri-state box verdict (InputBoxState), VerifyFillAgainstPrompt (exact / wrap-tolerant / collapse-aware / Partial / Foreign incl. the Incident-3 placeholder hazard), DecideSendUndo, the R7 merge classifier (push + pull; Failed + autorunner paused), the DecideAdvance/DecideEnterRetry box-state holds, ShouldPauseOnBoxNotVisible, SetPendingBoxState notify semantics, the verifySendBeforeSubmit round-trip
