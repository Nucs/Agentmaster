...
namespace winrt::TerminalApp::implementation
{
    ...
    private:
        ...
        void _HandleCommandHandover(const std::wstring& sessionId, const std::wstring& mdPayload, bool inPlace = false); // Agentmaster (COMMANDS.md): a /handover-family await resolved — if THIS window hosts the origin session's tab, create the "<title> (handover)" successor and deliver the CONTENT of EVERY collected markdown (mdPayload = the '|'-joined path set, SplitWatchPaths; one command may write several HANDOVER-*.md files — joined in write order into ONE first user message) via the delivery tiers (commandline under the escaped budget; full-document bracketed-paste injection above it; NEVER truncated). inPlace=false (/handover) spawns the successor in a NEW tab beside the origin; inPlace=true (/handover-here) REPLACES the origin tab via _RestartTabIntoFreshSession, degrading to the new-tab spawn if the in-place swap is unavailable
        std::wstring _RestartTabIntoFreshSession(const TerminalApp::Tab& originTab, const ::Agentmaster::SessionInfo& origin, const std::wstring& dir, const std::wstring& title, const std::wstring& initialPrompt); // Agentmaster (COMMANDS.md — /handover-here): REPLACE the origin session's live pane in place with a brand-new FRESH conversation — "Open New Session Here -> Default" spawn semantics (new minted id, the settings model, same dir) delivered through the Restart-session swap (_RestartManagedSession's recipe: HardResetWithoutErase + Connection swap + Start, scrollback kept); _BindClaudeSessionToTab's re-home block then archives the origin (live=false, injector cleared, resumable from Sessions) and re-keys the tab onto the successor. Returns the successor's session id ("" == refused/failed — the caller falls back to the new-tab spawn so the handover is never lost). UI thread only
