...
namespace winrt::TerminalApp::implementation
{
    ...
    private:
        ...
        void _HandleCommandHandover(const std::wstring& sessionId, const std::wstring& mdPath, bool inPlace = false); // Agentmaster (COMMANDS.md): a /handover-family await resolved — if THIS window hosts the origin session's tab, create the "<title> (handover)" successor and deliver the markdown's CONTENT as its first user message (commandline tier under the escaped budget; full-document bracketed-paste injection above it; NEVER truncated). inPlace=false (/handover) spawns the successor in a NEW tab beside the origin; inPlace=true (/handover-here) REPLACES the origin tab via _RestartTabIntoFreshSession, degrading to the new-tab spawn if the in-place swap is unavailable
