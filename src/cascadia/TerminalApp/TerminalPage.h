...
namespace winrt::TerminalApp::implementation
{
    ...
    private:
        ...
        void _MoveSessionTriageState(const std::wstring& sessionId); // Agentmaster (Waiting-for-you triage): the tab context-menu's status-adaptive move — demote a WaitingForInput session to Idle ("Move to Idle/Done") or plainly promote an Idle/Done one to WaitingForInput ("Move to Waiting-for-you"); direction re-derived from the live registry state. EXPLICITLY separate from _MarkSessionUnread (no sticky manualUnread, no ring flash)
        ...
        void _SetTabAgentFavorite(const TerminalApp::Tab& tab, bool on); // Agentmaster (FAVORITES.md): show/hide the gold FAVORITE crown over a tab's status dot via Tab.TabStatus().AgentFavoriteVisible; UI thread, idempotent
        void _RefreshTabFavoriteCrown(const std::wstring& sessionId); // Agentmaster (FAVORITES.md): re-read IsSessionFavorite(sid) and (re)assert the crown on this window's hosting tab; no-op when this window doesn't host the session. Called at bind/launch + on toggle (same-window instant; cross-window catches up on next bind)
