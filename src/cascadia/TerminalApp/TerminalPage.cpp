...
#undef ON_ALL_ACTIONS
    ...
    void TerminalPage::_RegisterTabEvents(Tab& hostingTab)
    {
        ...
        // tab uses its EFFECTIVE work dir (EffectiveWorkingDir — the inferred dir under the Inferred
        // tab-color mode, else its recorded workingDir; a claude/codex tab emits no OSC cwd, so the
        // live control cwd would be empty), matching the Manager's Open-New-Session-Here; otherwise
        ...
        hostingTab.NewSessionHereRequested([weakTab, weakThis](winrt::hstring model) {
            ...
            if (const auto sid = page->_ClaudeSessionForTab(*tab); !sid.empty() && page->_sessionRegistry)
            {
                ...
                {
                    dir = ::Agentmaster::EffectiveWorkingDir(page->_appSettings.tabColorMode, *s);
    ...
    safe_void_coroutine TerminalPage::RequestQuit()
    {
        ...
        // deterministic quit seam (symmetric to CloseWindow) while it is still laid out, so the
        // initiating window's final geometry / tab order / focus / lens reaches disk. The OTHER windows
        // torn down by QuitRequested persist via the ~TerminalPage catch-all. Latch so our own
        // destructor won't re-capture after the teardown-archive clears _claudeTabs (it would clobber).
        _FlushWindowRecord();
    ...
    safe_void_coroutine TerminalPage::CloseWindow()
    {
        ...
        _FlushWindowRecord();
        _windowRecordTeardownFlushed = true; // claimed the deterministic flush — the destructor must NOT re-capture (it would clobber this record after the archive below clears _claudeTabs)
